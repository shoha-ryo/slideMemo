...
import { 
  Loader2, ShieldCheck, UserPlus, XCircle, ChevronRight, UserCircle 
} from "lucide-react";
    ...
    return (
      <div className="flex h-screen items-center justify-center p-4 bg-slate-50">
        <Card className="w-full max-w-md border-red-100 shadow-lg">
          ...
          <CardFooter>
            <Button onClick={() => router.push("/")} className="w-full">トップページへ</Button>
          </CardFooter>
        </Card>
      </div>
    );
			...
			<div className="min-h-screen bg-slate-50 flex items-center justify-center p-4">
				<Card className="w-full max-w-lg shadow-2xl border-none">
					<CardHeader className="text-center pb-2">
						...
						<CardTitle className="text-2xl font-bold">プロジェクトへの招待</CardTitle>
						<CardDescription>新しいチームがあなたの参加を待っています</CardDescription>
					</CardHeader>

					<CardContent className="space-y-6 pt-6">
						{/* 1. 招待元の情報セクション（プロジェクトと招待者のみ） */}
						<div className="space-y-2">
							<label className="text-[10px] font-bold text-slate-400 uppercase tracking-widest ml-1">招待元</label>
							<div className="bg-white border-2 border-slate-100 rounded-2xl p-5 shadow-sm space-y-4">
								<div className="flex flex-col">
									...
									<span className="text-xs text-slate-400 font-medium mt-1">プロジェクトに招待されました</span>
								</div>
								...
								<div className="flex items-center gap-3 pt-3 border-t border-slate-50">
									...
									<div className="flex flex-col min-w-0">
										<span className="text-[10px] text-slate-400 font-bold uppercase">招待者</span>
										<span className="text-sm font-semibold text-slate-700 truncate">
											{inviteData?.inviterName || "管理者"}
										</span>
										<span className="text-[10px] text-slate-400 truncate">{inviteData?.inviterEmail}</span>
									</div>
								</div>
							</div>
						</div>
						...
						<div className="space-y-2">
							<label className="text-[10px] font-bold text-slate-400 uppercase tracking-widest ml-1">参加するあなたのアカウント</label>
							<div className="bg-amber-50/50 border border-amber-100 rounded-2xl p-4 space-y-3">
								<div className="flex items-center gap-3">
									...
									<div className="flex-1 min-w-0">
										<p className="text-[10px] font-bold text-amber-800 uppercase">ログイン中</p>
										<p className="text-sm text-amber-900/70 truncate font-bold">{user?.name}</p>
										<p className="text-sm text-amber-900/70 truncate font-medium">{user?.email}</p>
									</div>
									<Button variant="outline" size="sm" className="text-[10px] h-7 px-2 bg-white border-amber-200 text-amber-700 hover:bg-amber-100">
										切替
									</Button>
								</div>
