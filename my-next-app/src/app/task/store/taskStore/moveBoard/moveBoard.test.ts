...
describe("moveBoardLogic", () => {
  const mockInitialState: AppState = {
    ...
    boards: {
      "board-1": { id: "board-1", title: "B1", cardIds: ["card-1"], projectId: "p1" },
  ...
  it("ボードを別のボードの位置へ正しく移動できること (1を3の位置へ)", () => {
    const payload: Payload = { activeId: "board-1", overId: "board-3", dropPosition: null };
  ...
  it("ボードをカードの上にドロップしたとき、そのカードの所属ボードをターゲットにすること", () => {
    ...
    const payload: Payload = { activeId: "board-3", overId: "card-1", dropPosition: null };
    const result = moveBoardLogic(payload, mockInitialState);
  ...
  it("移動元と移動先が同じ場合は、何もしないこと", () => {
    const payload: Payload = { activeId: "board-1", overId: "board-1", dropPosition: null };
    const result = moveBoardLogic(payload, mockInitialState);
  ...
  it("存在しないIDが含まれる場合、何もしないこと", () => {
    const payload: Payload = { activeId: "board-1", overId: "ghost-board", dropPosition: null };
    const result = moveBoardLogic(payload, mockInitialState);
