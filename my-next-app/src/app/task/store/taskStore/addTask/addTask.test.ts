...
import { AppState, Source } from "@/types/task";
...
describe("addCardLogic", () => {
  const mockInitialState: AppState = {
    boardOrder: ["board-1"],
    boards: {
      "board-1": { id: "board-1", title: "Board 1", cardIds: [], projectId: "p1" },
    },
    cards: {
      "existing-card": {
        id: "existing-card",
  ...
  it("ボードに対して新しいカードを正常に追加できること", () => {
    ...
    const source: Source = { type: "board", data: mockInitialState.boards["board-1"] };

    const result = addCardLogic(title, source, mockInitialState);

    // 新しく生成されたカードのIDを特定
    const newCardId = Object.keys(result.newState.cards).find(id => id.startsWith("card-"))!;
  ...
  it("既存のカードに対して子カードとして追加できること", () => {
    ...
    const source: Source = { type: "card", data: mockInitialState.cards["existing-card"] };

    const result = addCardLogic(title, source, mockInitialState);

    const newCardId = Object.keys(result.newState.cards).find(id => id.startsWith("card-") && id !== "existing-card")!;
    
    // 子カード側の parentId が正しいか
    expect(result.newState.cards[newCardId].parentId).toBe("existing-card");
    // 親カード側の childrenIds に追加されているか
    expect(result.newState.cards["existing-card"].childrenIds).toContain(newCardId);
  });
