...
describe("deleteCardLogic", () => {
  ...
  const mockInitialState = {
    boardOrder: ["board-1"],
    boards: {
      "board-1": { id: "board-1", title: "Board 1", cardIds: ["card-a"], projectId: "p1" },
    },
    cards: {
      "card-a": { 
        id: "card-a", title: "Parent", boardId: "board-1", parentId: null, childrenIds: ["card-b"] 
      } as CardType,
      "card-b": { 
        id: "card-b", title: "Child", boardId: "board-1", parentId: "card-a", childrenIds: ["card-c"] 
      } as CardType,
      "card-c": { 
        id: "card-c", title: "Grandchild", boardId: "board-1", parentId: "card-b", childrenIds: [] 
  ...
  it("カードを削除したとき、その子孫もすべて state から消え、親の参照が更新されること", () => {
    ...
    const result = deleteCardLogic(targetId, mockInitialState as unknown as TaskStore);
  ...
  it("存在しない cardId の場合は何もせず emptyTasks を返すこと", () => {
    const result = deleteCardLogic("unknown", mockInitialState as unknown as TaskStore);
