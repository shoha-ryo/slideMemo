...
export async function getInitialData(
  ...
  if (lastSyncAt === 0) {
    return {
      diffTasks: {
        ...
        deleteTasks: { boardOrder: [], boardIds: [], cardIds: [], labelIds: [] },
      },
  ...
  const ids = {
		...
		label: { create: new Set<string>(), update: new Set<string>(), delete: new Set<string>() },
	}; // [new Set<string>]はダブりを自動排除してくれる

  project.activityLog.forEach((log) => {
		const type =
			log.entityType === "CARD"  ? "card"  :
			log.entityType === "BOARD" ? "board" :
			log.entityType === "LABEL" ? "label" :
			null;
