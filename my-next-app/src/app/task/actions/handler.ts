...
export const handleKeyDown = (e: React.KeyboardEvent, showModal: Pick<ModalStore, "showModal">["showModal"]) => {
	...
	switch (e.key) {
		case "ArrowDown": {
			...
			if (current.classList.contains("card")) {
				...
				let parent = current.parentElement?.closest(".card") as HTMLElement | null;
				while (parent) {
					const parentNextSibling = parent.nextElementSibling as HTMLElement;
					if (parentNextSibling && parentNextSibling.classList.contains("card")) {
		...
		case "ArrowUp": {
			...
			if (current.classList.contains("card")) {
				...
				if (prevSibling && prevSibling.classList.contains("card")) {
					...
					let lastChild = prevSibling.querySelector(".card:last-of-type") as HTMLElement;
					if (lastChild) {
						lastChild.focus();
					} else {
						prevSibling.focus();
				...
				const parentCard = current.parentElement?.closest(".card") as HTMLElement;
				if (parentCard) {
					parentCard.focus();
					break;
				}
		...
		case "ArrowRight": {
			...
			const nextBoard = currentBoard?.nextElementSibling?.closest('.board') as HTMLElement;
			nextBoard?.focus();
			break;
		}
		case "ArrowLeft": {
			e.preventDefault();
			const currentBoard = current.closest('.board');
			const prevBoard = currentBoard?.previousElementSibling?.closest('.board') as HTMLElement;
			prevBoard?.focus();
			break;
		}
		case "Enter": {
