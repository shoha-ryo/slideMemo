...
export const ThemeSidebar = ({ activeSidebar, onToggle }: Props) => {
					...
					<div className="flex-1 overflow-y-auto p-4 space-y-3 custom-scrollbar">
						{themes.map((t) => (
							...
							>
								...
								>
									<div className="h-4 w-4 rounded-full shadow-sm" style={{ backgroundColor: t.primary }} />
								</div>

								{/* ラベル */}
								<div className="flex-1 text-left">
									<p className="text-sm font-medium">{t.name}</p>
									<p className="text-[10px] text-muted-foreground uppercase">{t.id}</p>
								</div>
