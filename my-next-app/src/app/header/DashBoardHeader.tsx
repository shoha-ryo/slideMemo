...
import { Search, Bell, Settings, Menu, CheckSquare } from "lucide-react"
					...
					<DropdownMenu>
						...
						<DropdownMenuContent align="end" className="w-80">
							...
							<DropdownMenuItem className="flex flex-col items-start gap-1 p-3">
								<div className="font-medium">新しいタスクが割り当てられました</div>
								<div className="text-sm text-muted-foreground">「ランディングページのデザイン」が追加されました</div>
								<div className="text-xs text-muted-foreground">5分前</div>
							</DropdownMenuItem>
							<DropdownMenuItem className="flex flex-col items-start gap-1 p-3">
								<div className="font-medium">プロジェクトが更新されました</div>
								<div className="text-sm text-muted-foreground">田中さんが「Webアプリ開発」を更新しました</div>
						...
						<DropdownMenuTrigger asChild>
							<Button variant="ghost" className="h-9 gap-2 px-2">
								<Avatar className="h-7 w-7">
									...
									<AvatarFallback className="bg-muted text-xs">YS</AvatarFallback> {/* アバター画像がなければこちらを表示 */}
								</Avatar>
